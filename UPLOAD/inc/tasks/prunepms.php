 ...
 * @copyright 2022 MyBB Group <http://mybb.group>
...
$queryString =
    ...
    TABLE_PREFIX .
    "users u SET 
		totalpms = (SELECT COUNT(pmid) FROM " .
    TABLE_PREFIX .
    "privatemessages pm WHERE pm.uid = u.uid),
		unreadpms = (SELECT COUNT(pmid) FROM " .
    TABLE_PREFIX .
    "privatemessages pm WHERE
...
function task_prunepms($task)
{
    ...
    $db->delete_query(
        ...
        "(dateline<" .
            (TIME_NOW - $secs) .
            " AND readtime>0) OR dateline<" .
            (TIME_NOW - $usecs) .
            " AND (folder='1' OR folder='2' OR folder='3' OR folder='4') "
    );
    ...
    add_task_log($task, $lang->prunepms_task_log_optimise_table);
}